...
# EDIT BELOW: These are useful parameters you may want/need to change.
local_path_to_save_file="/Users/caseymeili/Desktop/centrifuge_diagram.png"   # Edit the path to save the file and the file name
...
for(tubes in 2:(slots_in_centrifuge-2)){
	...
	if(tubes%%2 == 0 & slots_in_centrifuge%%2 == 0){
		...
		for(f in filled){
			...
			points(x = 0 + 1 * cos(a), y = 0 + 1 * sin(a), pch=16, cex=slot_circle_size, col="#4AA2A5")
		}
	}
	...
	if(tubes%%3 == 0 & tubes%%2 > 0 & slots_in_centrifuge%%3 == 0){
		...
		for(f in filled){
			a=f*pi/(slots_in_centrifuge/2)
			points(x = 0 + 1 * cos(a), y = 0 + 1 * sin(a), pch=16, cex=slot_circle_size, col="#F86F31")
		}
	}
